...
let history = (function () {
  ...
  return {
    ...
    getForward: () => {
      ...
      indArr++;
      if (indArr >= MAX_LENGTH) {
        indArr = MAX_LENGTH - 1;
        return arr[MAX_LENGTH - 1];
      } else if (indArr >= arr.length) {
        indArr = arr.length - 1;
        return arr[indArr];
      } else {
        return arr[indArr];
 ...
 */
class TextexPrompt extends Prompt {
  constructor(opts={}) {
    super(opts);
  ...
  // Added by Yang Jun
  up() {
    this.value = this.history.getBackward();
    this.cursor = this.value.length;
    this.render();
  }
  down() {
    this.value = this.history.getForward();
    this.cursor = this.value.length;
    this.render();
  }
